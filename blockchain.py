...
# The "node" of the blockchain. Points to the
# previous block by its unique hash in previous_hash.
class Block():

    # Default data for block defined in constructor.
    # Minimum specified should be number and data.
    ...
    # Function instead of variable in constructor
...
class Blockchain():
    ...
    def isValid(self):
        ...
        for i in range(1, len(self.chain)):
            ...
            # compare the previous hash to the
            # actual hash of the previous block
            if ((_previous != _current) or (
                 _current[:self.difficulty] != "0"*self.difficulty)):
                return False
        return True
